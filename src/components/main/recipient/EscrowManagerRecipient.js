...
import ContractDataRetriever from "./ContractDataRetriever"
import DeployedEscrowRecipient from "./DeployedEscrowRecipient"

const wait = (delay) =>
    new Promise(resolve => setTimeout(resolve, delay));

export default class EscrowManagerRecipient extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            storageKey: base58Encode(keccak(props.passHash + "recipient")),
            rows: {}
        };
    }
    ...
    addRow(rowData) {
        ...
        let row = {
            ...
            counterparty: rowData.buyerAddress,
            sum: rowData.sum,
            blockUnlocked: rowData.blockUnlocked,
            status: "Open",
            contractPubkey: rowData.contractPubkey,
            verificationHash: rowData.verificationHash,
    ...
    updateRow(key, field, value) {
        ...
        let newRow = {
            ...
            sum: row.sum,
            blockUnlocked: row.blockUnlocked,
            status: row.status,
            contractPubkey: row.contractPubkey,
            ...
            <Table striped key="Tracker">
                ...
                <Table.Body>
                    {Object.keys(this.state.rows).map(key => {
                        return <DeployedEscrowRecipient{...this.state.rows[key]}
                                                       key={key}
                                                       chainConnector={this.props.chainConnector}/>
                    })}
                </Table.Body>
            </Table>,
            <ContractDataRetriever parentCallback={this.addRow} chainConnector={this.props.chainConnector}/>
        ]
